...
void newSite::renderSite(sf::RenderWindow &window, size_t index) {
  for (std::size_t i = 0; i < lineVec.size(); ++i) {
    ...
    if (lineVec[i]->isActive) {
      ...
      lineVec[i]->shape->rectangle.setFillColor(sf::Color(20, 20, 20));
    };
  ...
  for (std::size_t x = 0; x < textVec.size(); ++x) {
    ...
    if (textVec[x]->isActive) {
      ...
      textVec[x]->text.setFillColor(sf::Color::White);
    };
    textVec[x]->isActive = lineVec[x]->isActive;
    window.draw(textVec[x]->text);
  }

  if (selectAreaRect != nullptr) {
    sf::Vector2f curMousepos = sf::Vector2f(sf::Mouse::getPosition(window).x,
...
void newSite::selectArea(sf::Event event, sf::RenderWindow &window,
  ...
  if (event.type == sf::Event::MouseButtonPressed) {
    ...
    initMousepos = sf::Vector2f(sf::Mouse::getPosition(window).x,
                                sf::Mouse::getPosition(window).y);
  ...
  if (event.type == sf::Event::MouseButtonReleased) {
    ...
    for (size_t i = 0; i < lineVec.size(); ++i) {
      ...
      if ((selectionPos.y < rectPos.y && selectionPos.x < rectPos.x &&
           (selectionPos.y + selectionSize.y) > (rectPos.y + rectSize.y) &&
           (selectionPos.x + selectionSize.x) > (rectPos.x + rectSize.x)) ||
          (selectionPos.y > rectPos.y && selectionPos.x > rectPos.x &&
           (rectPos.y + rectSize.y) > (selectionPos.y + selectionSize.y) &&
           (rectPos.x + rectSize.x) > (selectionPos.x + selectionSize.x))) {
        if (!firstFound) {
          index = i;
          firstFound = true;
        }
